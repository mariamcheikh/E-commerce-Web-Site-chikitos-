...
include 'lib/Resev_trai.php';

if (isset($_POST['date_res']) && isset($_POST['per_res'])) {
	$trai = new reserv_trait();
	$trai->setId_client($_SESSION['user_session']);
	$_SESSION['id_trai'] = $_GET['insert'];
	$trai->setId_tra($_GET['insert']);
	$trai->setPeriode($_POST['per_res']);
	$trai->setDate_reserv($_POST['date_res']);
    $c = new  CrudFrontOff();
    $conn=$c->getConnexion();
    $c->Inserttrai($conn,$trai);
	header('Location:code_pdf/ExportPdf.php');
	die();
}
?>
<!DOCTYPE html>
	...
	<div class="container">
        ...
        <form id="registration-form" method="POST" class="form-horizontal" action="#">
          <div class="form-group">
            ...
            <div class="col-md-4">
              <input type="date" class="form-control" id="firstname" name="date_res" placeholder="Entrez la date" required >
            </div>
          </div>
          <div class="form-group">
            <label class="col-md-2 control-label" for="lastname">Periode</label>
            <div class="col-md-4">
              <input type="text" class="form-control" id="lastname" name="per_res" placeholder="Entrez la periode" required>
            </div>
          </div>
        </div>
        <button type="submit" id="bt_enr" name="Engr" class="btn" style="margin-left:50px;">Enregistrer</button>
      </div>
          </form>
			</div>
