	...
	<div class="breadcrumbs">
		<div class="container">
			<ol class="breadcrumb breadcrumb1 animated wow slideInLeft" data-wow-delay=".5s">
				...
				<li class="active">Réservation table</li>
			</ol>
		</div>
	</div>
  <br>
...
<!-- register -->
	...
	<div class="container">
        ...
        <option value="Numero">Organiser par Numero table</option>
        </select>
        </form>
        <table class="table table-striped">
                     ...
                     <tbody>
                       ...
                       <tr ng-repeat="comment in comments | filter:query | orderBy:order">
                         <td>
                           <form id="registration-form" method="GET" class="form-horizontal" action="AjoutReserv_table.php" style="margin-left:7px;">
                             <div class="form-group">
                               <button type="submit" id="Supp_b" name="Engr" class="btn btn-success" >Réserver</button>
                               <input type="text" name="InsertTable" value="{{comment.Id}}" hidden>
                             </div>
                           </form>
					...
					app.controller('commentsCtrl',function commentsCtrl ($scope) {console.log($scope);
						$scope.comments = [
							...
							?>
							{
								...
								"Numero":"<?php echo $data['Numero_table']; ?>",
								"etat":"<?php echo $data['Etat_table']; ?>",
							},
							<?php
							}
							?>
				...
				<script type="text/javascript">
					$(function () {
						$("#comm").submit(function () {
							...
							comm = $(this).find("textarea[name=commentaire]").val();
							//alert(pseudo+"--------"+comm);
							$.post("AddComments.php",{pseudo: pseudo,msg: comm},function(data){
								$("#loader").hide();
								// alert(data);
								$("#resultatAjax").hide().append(pseudo+" à ecrit :<div class=\"msg\">"+comm+"</div>"+"<hr>").slideDown();
							});
							return false;
						});
					});
				...
				<br>
				<?php
				$req2 = $c->DisplayComments($conn);
				while ($data = $req2->fetch()) {
					echo $data['pseudo']." à ecrit :";
					echo "<div class=\"comm\">";
					echo $data['contenu'];
					echo "</div>";
					echo "<hr>";
				...
				<hr>
				<form action="#" method="post" id="comm" class="forcomm">
					<p> Message </p>
					<input type="text" name="pseudo" value="<?php echo $_SESSION['user_name']; ?>" hidden>
					<textarea  name="commentaire" class="form-control"></textarea>
					<br>
					<button type="submit" id="Supp_b" name="commentaire" class="btn btn-success" >Ajouter votre commentaire</button>
					<div id="loader" style="display: none"><img src="images/ajax-loader.gif" alt="testing"></div>
				</form>
			</div>
