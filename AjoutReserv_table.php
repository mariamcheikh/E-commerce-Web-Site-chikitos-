...
if (isset($_POST['date_res']) && isset($_POST['nbr_per'])) {
	$tab = new reser_tab();
	$tab->setId_client($_SESSION['user_session']);
	$_SESSION['id_tab'] = $_GET['InsertTable'];
	$tab->setId_table($_GET['InsertTable']);
	$tab->setDate_reserv($_POST['date_res']);
	$tab->setNbr_pers($_POST['nbr_per']);
	$c = new  CrudFrontOff();
	$conn = $c->getConnexion();
    $c->Inserttabl($conn,$tab);
	header('Location:code_pdf/ExportPdf2.php');
	die();
}
//var_dump($_SESSION);
?>
	...
	<div class="container">
        ...
        <form id="registration-form" method="POST" class="form-horizontal" action="#">
          ...
          <div class="form-group">
            ...
            <div class="col-md-4">
              <input type="text" class="form-control" id="lastname" name="nbr_per" placeholder="Entrez la periode" required>
            </div>
          </div>
        </div>
